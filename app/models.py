...
class CrewQueryRequest(BaseModel):
    """Request for the CrewAI multi-agent query pipeline."""
    query: str = Field(..., min_length=1, max_length=2000, description="User query")
    top_k: int = Field(default=10, ge=1, le=30, description="Chunks to retrieve before crew runs")
    org_id: str = Field(default="default", description="Organisation namespace")
    verbose: bool = Field(default=False, description="Enable verbose crew logging")


class CrewQueryResponse(BaseModel):
    """Response from the CrewAI multi-agent pipeline."""
    query: str
    answer: str = Field(description="Verified, synthesized answer")
    confidence: float = Field(ge=0.0, le=1.0, description="Fact-checker confidence score")
    issues: List[str] = Field(default_factory=list, description="Unsupported claims flagged by fact checker")
    sources_used: List[str] = Field(default_factory=list, description="Source titles backing the answer")
    agents_used: List[str] = Field(description="Names of agents that ran")
    processing_time: float = Field(description="Wall-clock seconds for the full crew run")
