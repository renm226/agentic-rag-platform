...
@app.post("/query/crew", tags=["Multi-Agent QA"], response_model=CrewQueryResponse)
async def crew_query(
    request: CrewQueryRequest,
    db: AsyncSession = Depends(get_db),
):
    """
    Answer complex queries using a CrewAI multi-agent pipeline.

    Four sequential agents collaborate:
    - **Query Planner** — decomposes the question and identifies key concepts
    - **Retrieval Specialist** — curates the most relevant chunks from the initial
      retrieval (and can search for more via the /retrieve endpoint if API_BASE_URL is set)
    - **Answer Synthesizer** — builds a comprehensive cited answer
    - **Fact Checker** — verifies every claim and assigns a confidence score

    The crew runs in a thread pool so it never blocks the async event loop.
    For simple questions prefer `/query` or `/query/intelligent`; use this endpoint
    when the question is multi-part, comparative, or requires cross-document reasoning.
    """
    import asyncio
    ...
    try:
        ...
        retriever = AdvancedRetriever(
            session=db,
            org_id=org_id,
            top_k=request.top_k * 2,   # over-retrieve; crew will curate
            max_tokens=6000,
            use_bm25_rerank=True,
        )
        retrieval_results = await retriever.retrieve(query, query_embedding, request.top_k)
        ...
        log_error(logger, e, {"query": query[:100], "org_id": org_id})
        raise HTTPException(status_code=500, detail="CrewAI pipeline failed")
